...
class MakananItem extends MenuItem {
    ...
    private int levelKepedasan;
    
    public MakananItem(String nama, double harga, boolean isVegetarian, int levelKepedasan) {
        super(nama, "Makanan", harga);
        this.isVegetarian = isVegetarian;
        setLevelKepedasan(levelKepedasan);
    }
    ...
    public int getLevelKepedasan() { return levelKepedasan; }
    
    public void setLevelKepedasan(int levelKepedasan) {
        if (levelKepedasan < 0 || levelKepedasan > 5) {
            throw new IllegalArgumentException("Level kepedasan harus antara 0-5!");
        }
        this.levelKepedasan = levelKepedasan;
    }
    
    @Override
    public String getDeskripsi() {
        String vegetarianStatus = isVegetarian ? "Vegetarian" : "Non-Vegetarian";
        return String.format("%s (%s, Level Pedas: %d/5)", getNama(), vegetarianStatus, levelKepedasan);
    }
    
    @Override
    public String toString() {
        return String.format("%s | %s | Level Pedas: %d/5", 
                super.toString(), isVegetarian ? "Vegetarian" : "Non-Vegetarian", levelKepedasan);
    }
}
...
public class RestaurantEase {
    ...
    private static void initSampleData() {
        ...
        menuItems.add(new MakananItem("Croissant", 15000, false, 0));
        menuItems.add(new MakananItem("Cheesecake", 28000, true, 0));
    ...
    private static void tambahMenuMakanan() {
        ...
        int levelPedas = getIntInput("Level kepedasan (0-5): ");
        
        MenuItem newItem = new MakananItem(nama, harga, isVegetarian, levelPedas);
        menuItems.add(newItem);
    ...
    private static void editMenuMakanan(MakananItem item) {
        ...
        System.out.print("Ubah level kepedasan? (y/n): ");
        String ubahPedas = scanner.nextLine();
        if (ubahPedas.equalsIgnoreCase("y")) {
            int newLevel = getIntInput("Level kepedasan baru (0-5): ");
            item.setLevelKepedasan(newLevel);
        }
    ...
    private static void buatPesananBaru() {
        ...
        while (!selesaiMemesan) {
            ...
            switch (pilihan) {
                case 1:
                    tambahItemKePesanan(pesananBaru);
                    break;
                case 2:
